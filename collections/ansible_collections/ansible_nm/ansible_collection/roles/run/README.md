Ansible_nm.Ansible_collection Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: ansible_nm.ansible_collection.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from ansible_nm.ansible_collection
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: ansible_nm.ansible_collection.run
      vars:
        run_x: 42
```
